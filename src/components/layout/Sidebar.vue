<template>
  <aside class="w-full">
      ...
      <div class="p-3">
        <ul class="divide-y divide-light-200">
          <li v-for="(news, index) in popularNews" :key="news.id" class="py-3">
            ...
            >
              <div class="flex-shrink-0 w-16 h-16 bg-light-100 rounded overflow-hidden">
                <img 
                  :src="news.image" 
                  :alt="news.title"
              ...
              <div class="flex-grow">
                <span class="text-xs text-primary">{{ formatDate(news.date) }}</span>
                <h4 class="text-sm font-medium text-dark group-hover:text-primary transition-colors line-clamp-2">
                  {{ news.title }}
                </h4>
              </div>
              <div class="flex-shrink-0 w-8 h-8 bg-light-100 rounded-full flex items-center justify-center text-sm font-semibold text-primary">
                {{ index + 1 }}
              </div>
            </router-link>
          </li>
      ...
      <div class="p-4">
        <div class="flex flex-wrap gap-2">
          <a 
            ...
            :key="tag.id" 
            href="#"
            class="px-3 py-1.5 bg-light-100 hover:bg-primary hover:text-white rounded-full text-sm transition-colors"
          >
            {{ tag.name }}
          </a>
        </div>
    ...
    <!-- Newsletter -->
    <div v-if="showNewsletter" class="bg-primary rounded-lg shadow-sm mb-6 text-white">
      <div class="p-5">
